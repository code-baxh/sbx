...
function testMailNotification(){
	...
	try {	
	    ...
	    return "Message could not be sent. Mailer Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
	}	
}
function chatMailNotification($user_id,$email_message,$s_id=0){
	...
	try {
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Chat mail notification to '.$sm['mail']['email']." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
...
function fanMailNotification($sexy_id){
	...
	try {	
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Like mail notification to '.$sm['mail']['email']." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}
...
function forgotMailNotification($name,$email,$link){
	...
	try {	
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Recover password email to '.$email." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}
...
function matchMailNotification($sexy_id){
	...
	try {	
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Match mail notification to '.$sm['mail']['email']." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}
...
function nearMailNotification($sexy_id,$photo){
	...
	try {
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Near mail notification to '.$sm['mail']['email']." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}
}
function welcomeMailNotification($user_name,$user_email,$user_password){
	...
	try {	
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Welcome mail notification to '.$user_email." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}
...
function welcomeMailVerification($user_name,$user_id,$user_email,$user_password){
	...
	try {	
		...
		if($sm['plugins']['logActivity']['enabled'] == 'Yes'){
			$activity = 'Welcome mail verification notification to '.$user_email." could not be sent, Error: {$mail->ErrorInfo} HOST :$sm[plugins][email][host] PORT: $sm[plugins][email][port] Protection :$sm[plugins][email][protection]";
			activity('system',$activity,'Email could not be sent');
		}
	}	
}
...
function discoverStoriesMobile($lat,$lng,$looking,$filters=''){
	...
	$storyFrom = $sm['plugins']['story']['days'];
	$time = time();	
	$extra = 86400 * $storyFrom; //Stories which are of one day before *Malik Ahsan* 
	$storyFrom = $time - $extra;
